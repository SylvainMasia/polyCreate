...
public class RotateAspect extends ActionAspect {
  @Step
  @ReplaceAspectMethod
  public static void runIt(final Rotate _self, final PolyCreateControler controler, final SensorChecker logicalVerifications) {
    ...
    if (_self instanceof fr.univcotedazur.webots.polycreate.Rotate){
    	fr.inria.diverse.k3.al.annotationprocessor.stepmanager.StepCommand command = new fr.inria.diverse.k3.al.annotationprocessor.stepmanager.StepCommand() {
    		@Override
    		public void execute() {
    			fr.univcotedazur.webots.polycreate.rewritingrules.RotateAspect._privk3_runIt(_self_, (fr.univcotedazur.webots.polycreate.Rotate)_self,controler,logicalVerifications);
    		}
    	};
    	fr.inria.diverse.k3.al.annotationprocessor.stepmanager.IStepManager stepManager = fr.inria.diverse.k3.al.annotationprocessor.stepmanager.StepManagerRegistry.getInstance().findStepManager(_self);
    	if (stepManager != null) {
    		stepManager.executeStep(_self, new Object[] {controler,logicalVerifications}, command, "Rotate", "runIt");
    	} else {
    		command.execute();
    	}
    	;
  ...
  protected static void _privk3_runIt(final RotateAspectRotateAspectProperties _self_, final Rotate _self, final PolyCreateControler controler, final SensorChecker logicalVerifications) {
    int _degrees = _self.getDegrees();
    float _divide = (((float) _degrees) / 100);
    final double robotDegrees = (_divide * 2.11);
